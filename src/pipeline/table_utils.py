...
def is_html_table_like(label_or_document: Any) -> bool:
    ...
    if isinstance(label_or_document, CanonicalBlock):
        ...
        if str(label_or_document.sub_type or "").strip().lower() == "flowchart":
            return False
        return bool(_first_parseable_table_candidate(_block_html_candidates(label_or_document)))
...
def extract_best_html_table_candidate(
    ...
    for block in sorted(
        ...
        if not is_html_table_like(block):
            continue
        parsed_candidate = _first_parseable_table_candidate(_block_html_candidates(block))
        if parsed_candidate is None:
            continue
        html_text, table_ir = parsed_candidate
        candidate = {
            "block": block,
            "html": html_text,
            "table_ir": table_ir,
...
def _block_html_candidates(block: CanonicalBlock) -> list[str]:
...
def _first_parseable_table_candidate(candidates: list[str]) -> tuple[str, Any] | None:
